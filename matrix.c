              ...
              int* color, int* distance)
{
    ...
    while(!done) {
        ...
        // the source vertex)
	for(int i = 0; i < cols; i++) // can use rows or cols, make sure they are the same step-condition in BOTH loops!
	{
		if (src[i] == 1) // unvisited
		{
			for(int j = 0; j < cols; j++)
			{ //SPMV 
				if(mat_trans[i][j] == 1 && color[j] == 0) // if we are at a valid vertex that is not visted....
				{
					dst[j] = 1;
					color[j] = 1;
					distance[j] = iter;
	...
	} //This is the second part of the SPMV
	int neighbor = 0; // got this idea and loop from stackoverflow, I used SPMV and BFS as my search words.
	for(int i = 0; i < rows; i++)
	{
		if(dst[i] == 1)
		{
			neighbor++; // all we need is one neighbor to consider this part finished, set done to 1 if no neighbors
			break;  
		}
	}
	if (neighbor == 0)
