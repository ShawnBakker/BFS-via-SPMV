...
void construct_adj_list(int** adj_mat, int rows, int cols, adj_node_t*** list)
{
  ...
  //    adj_node_t** myList = *list;
	...
	{
		if(adj_mat[i][j] != 0) // any value that isn't 0 in case of a loop or undirected graph.
		{
			adj_node_t* tmp = create_node(j); // tmp node that can be added to the list
			add_node(*list, i, tmp);	// list takes the new node, vid is just i
		}
	}	
    }
         ...
         int* color, int* distance, int* parent)
{
    ...
    while(src != NULL)
    {
	    int u = remove_node(&src);
	    adj_node_t* tmp = list[u];
	    while(tmp != NULL){
		    int v = tmp->vid;
		    printf("v : %d\n", v);
		    if (color[v] == 0){
			    ...
			    adj_node_t* tmp_node = create_node(v);
			    add_node(&list[u], 0, tmp_node);
